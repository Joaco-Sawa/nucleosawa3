...
import {
  ...
  User, Calendar, Phone, Mail, ChevronLeft, HelpCircle, FileText
} from 'lucide-react';
...
import catalogIcon from 'figma:asset/a1d61e7aee26ae9e65cfcc5d4d13290cc8da135b.png';
import walletIcon from 'figma:asset/f53abab596a88a7dbe37a1de9340df1fc709bfa0.png';
import shoppingBagIcon from 'figma:asset/5430a27800876129e7c102990a58d556b6b36a63.png';
...
export function ProfileView({ userPoints }: ProfileViewProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    telefono:         '+56 9 123',   // invalid — demo error state
  ...
  const [passwordData, setPasswordData] = useState({ nueva: 'pass', confirmar: 'otra' });
  const [passwordErrors, setPasswordErrors] = useState({
    nueva:     'La contraseña debe tener al menos 8 caracteres',
    confirmar: 'Las contraseñas no coinciden. Inténtalo de nuevo',
  });
  ...
  return (
    <motion.div
      ...
      className="flex-1 flex flex-col min-w-0 w-full max-w-full md:h-[calc(100vh-1rem)] md:overflow-y-auto md:m-2 md:rounded-2xl overflow-y-auto overflow-x-hidden pb-24 md:pb-12 bg-[#F5F8FB]"
    >
      ...
      <header className="md:hidden bg-white flex-shrink-0">
        ...
        <div className="px-[16px] py-3 flex items-center gap-3">
          <button
          ...
          <img src={logoDesafioSawa} alt="Desafío Sawa" className="h-9 object-contain flex-shrink-0" />
      ...
      {/* ── Content ── */}
      <div className="max-w-3xl mx-auto px-4 md:px-8 pt-8 md:pt-12 pb-2 w-full">
        ...
        >
          <div className="flex flex-col md:flex-row items-center md:items-center gap-4 md:gap-6">
            <div
              ...
              onMouseEnter={() => isEditing && setIsHoveringAvatar(true)}
              onMouseLeave={() => setIsHoveringAvatar(false)}
              onClick={() => isEditing && console.log('Open file picker')}
            >
              <img
                src={profileImage}
                alt="Carlos Toledo"
                className={`w-20 h-20 rounded-full ring-2 ring-slate-200 object-cover flex-shrink-0 transition-all ${isEditing ? 'cursor-pointer' : ''}`}
              />
              {isEditing && (
        ...
        <div className="mt-4 md:mt-6 mb-8 flex justify-center">
          <button className="px-16 py-2.5 text-slate-700 hover:text-slate-900 bg-white hover:bg-slate-50 border border-slate-300 rounded-full font-medium text-sm transition-all active:scale-95">
            Cerrar sesión
          </button>
        </div>
      ...
      {/* ── Bottom Navigation (Mobile) — ningún ítem activo ── */}
      <nav
        className="md:hidden fixed left-3 right-3 bg-white/90 backdrop-blur-xl rounded-[32px] z-50 border border-slate-200/50"
        style={{
