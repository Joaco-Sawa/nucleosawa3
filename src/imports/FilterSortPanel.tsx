...
export function FilterSortPanel({ isOpen, onClose, brands, onApply, buttonRef }: FilterSortPanelProps) {
  ...
  useEffect(() => {
    if (isOpen && buttonRef?.current) {
      ...
      setPosition({
        top: rect.bottom + 8,
        left: rect.right - 360  // Align to the right edge of button, 360px is panel width
      });
    }
  }, [isOpen, buttonRef]);
                    ...
                    <div className="px-4 pb-4 pt-2 bg-white border-t border-slate-100">
                      ...
                      <div className="mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => setMinPoints(Math.max(0, minPoints - 1000))}
                          ...
                          />
                          <button
                            onClick={() => setMinPoints(minPoints + 1000)}
                            className="w-10 h-10 rounded-lg bg-[#FF8000] hover:bg-[#FF8000]/90 flex items-center justify-center transition-colors active:scale-95"
                          >
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => setMaxPoints(Math.max(0, maxPoints - 1000))}
                            className="w-10 h-10 rounded-lg bg-[#FF8000] hover:bg-[#FF8000]/90 flex items-center justify-center transition-colors active:scale-95"
                          >
                            <Minus className="w-5 h-5 text-white" />
                          </button>
                          ...
                          />
                          <button
                            onClick={() => setMaxPoints(maxPoints + 1000)}
                            className="w-10 h-10 rounded-lg bg-[#FF8000] hover:bg-[#FF8000]/90 flex items-center justify-center transition-colors active:scale-95"
                          >
                            <Plus className="w-5 h-5 text-white" />
                          </button>
      ...
      <div className="hidden md:block">
        {isOpen && (
          <>
            ...
            <div 
              ref={panelRef}
              className="fixed z-50 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200"
              style={{
                top: `${position.top}px`,
                      ...
                      <div className="mb-2">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <input
                            ...
                            value={minPoints}
                            onChange={(e) => setMinPoints(Math.max(0, parseInt(e.target.value) || 0))}
                            className="flex-1 px-2 py-2 rounded-lg border border-slate-200 bg-white text-center text-xs font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#FF8000]/20 focus:border-[#FF8000] transition-all"
                            style={{ fontFamily: "'Nunito', sans-serif" }}
                          />
                          <button
                            onClick={() => setMinPoints(minPoints + 1000)}
