...
¡Comienza tu año trazando la ruta al éxito!`,
    ...
    type: 'concurso',
    contestStartDate: '2026-03-12', // Fecha de inicio del concurso
...
export function NewsDetail() {
  ...
  const commentsRef = useRef<HTMLDivElement>(null); // Ref para la sección de comentarios
  
  // Scroll automático a comentarios si hay hash #comentarios en la URL
  useEffect(() => {
    if (location.hash === '#comentarios' && commentsRef.current) {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 w-full max-w-full md:h-[calc(100vh-1rem)] md:overflow-y-auto md:m-2 overflow-y-auto overflow-x-hidden pb-[88px] md:pb-0">
      ...
      >
        ...
        <div className="hidden md:block py-[25px] px-6">
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => navigate(article.type === 'concurso' ? '/muro?filter=concurso' : '/muro')}
      ...
      <div className="max-w-4xl mx-auto w-full px-4 md:px-8 py-6">
        ...
        <div id="comentarios" ref={commentsRef} className="bg-slate-50 rounded-2xl p-4 md:p-6">
