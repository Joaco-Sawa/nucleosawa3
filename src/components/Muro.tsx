...
const newsArticles: NewsArticle[] = [
  {
    id: 15,
    title: '🎉 ¡Nuevo récord de participación en desafíos!',
    image: 'https://images.unsplash.com/photo-1552664730-d307ca884978?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
    excerpt: 'Este mes batimos récords con más de 500 colaboradores participando activamente.',
    ...
    publishedDate: '2026-03-24T18:30:00',
  ...
  {
    id: 16,
    title: '📢 Recordatorio: Webinar de liderazgo hoy a las 3 PM',
    image: null,
    excerpt: 'No te pierdas nuestra sesión sobre gestión de equipos remotos con expertos internacionales.',
    author: 'Desarrollo de Talento',
    authorImage: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?w=150',
    date: '24 de marzo de 2026',
    publishedDate: '2026-03-24T13:00:00',
...
export function Muro() {
  ...
  const [filterType, setFilterType] = useState<'todas' | 'noticia' | 'concurso'>('todas');
  const [newsSubfilter, setNewsSubfilter] = useState<'todas' | 'informativos' | 'desafios' | 'catalogo' | 'encuestas'>('todas');
  ...
  // Resetear subfiltro cuando se cambia el filtro principal
  useEffect(() => {
    if (filterType !== 'noticia') {
      setNewsSubfilter('todas');
    }
  }, [filterType]);
  ...
  const filteredNews = newsArticles.filter(article => {
    ...
    // Subfiltro de noticias (solo aplica cuando filterType === 'noticia')
    let matchesNewsSubfilter = true;
    if (filterType === 'noticia' && newsSubfilter !== 'todas') {
      const subcategoryMap: { [key: string]: string[] } = {
        'informativos': ['Logros del equipo', 'Beneficios', 'Sostenibilidad', 'Cultura organizacional'],
        'desafios': ['Desarrollo profesional'],
        'catalogo': ['Catálogo'],
        'encuestas': ['Encuestas']
      };
      const allowedCategories = subcategoryMap[newsSubfilter] || [];
      matchesNewsSubfilter = allowedCategories.includes(article.category);
    }
    
    return matchesSearch && matchesDateRange && matchesType && matchesNewsSubfilter;
  });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="hidden md:block py-[25px] px-6">
          <div className="flex items-center gap-3">
            {/* Filtros de tipo agrupados (Todo/Noticias/Concursos/Encuestas) */}
            <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-full p-1">
              <button
                onClick={() => setFilterType('todas')}
                className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${
                  filterType === 'todas' ? 'bg-[#FF8000] text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'
                }`}
              >
                Todo
              </button>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${
                  filterType === 'concurso' ? 'bg-[#FF8000] text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'
                }`}
              >
                Concursos
              </button>
        ...
        <div className="md:hidden">
          ...
          <div className="px-4 py-3 bg-[#F5F8FB]">
            {/* Filtros de tipo (Todo/Noticias/Concursos/Encuestas) */}
            <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-full p-1 mb-3 overflow-x-auto scrollbar-hide">
              <button
                onClick={() => setFilterType('todas')}
                className={`flex-shrink-0 px-2 py-2 rounded-full text-xs font-bold transition-all ${
                  filterType === 'todas' ? 'bg-[#FF8000] text-white shadow-sm' : 'text-slate-600 active:bg-slate-50'
                }`}
              >
                Todo
              </button>
              ...
              <button
                onClick={() => setFilterType('concurso')}
                className={`flex-shrink-0 px-2 py-2 rounded-full text-xs font-bold transition-all ${
                  filterType === 'concurso' ? 'bg-[#FF8000] text-white shadow-sm' : 'text-slate-600 active:bg-slate-50'
                }`}
              >
                Concursos
              </button>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 pt-4 md:pt-6 pb-6">
        ...
        {/* Pills de subcategorías de Noticias - Solo visible cuando filterType === 'noticia' */}
        {filterType === 'noticia' && (
          <motion.div 
            ...
            className="flex items-center gap-2 mb-4 md:mb-6 overflow-x-auto scrollbar-hide"
          >
            <motion.button
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ delay: 0.05, duration: 0.2 }}
              onClick={() => setNewsSubfilter('todas')}
            ...
            <motion.button
              ...
              onClick={() => setNewsSubfilter('informativos')}
              className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-semibold transition-all ${
                newsSubfilter === 'informativos' 
                  ? 'bg-slate-700 text-white shadow-sm' 
                  : 'bg-slate-200 text-slate-600 hover:bg-slate-300'
              }`}
            >
            ...
            <motion.button
              ...
              onClick={() => setNewsSubfilter('desafios')}
              className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-semibold transition-all ${
                newsSubfilter === 'desafios' 
                  ? 'bg-slate-700 text-white shadow-sm' 
                  : 'bg-slate-200 text-slate-600 hover:bg-slate-300'
              }`}
            >
            ...
            <motion.button
              ...
              onClick={() => setNewsSubfilter('catalogo')}
              className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-semibold transition-all ${
                newsSubfilter === 'catalogo' 
                  ? 'bg-slate-700 text-white shadow-sm' 
                  : 'bg-slate-200 text-slate-600 hover:bg-slate-300'
              }`}
            >
            ...
            <motion.button
              ...
              onClick={() => setNewsSubfilter('encuestas')}
              className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-semibold transition-all ${
                newsSubfilter === 'encuestas' 
                  ? 'bg-slate-700 text-white shadow-sm' 
                  : 'bg-slate-200 text-slate-600 hover:bg-slate-300'
              }`}
            >
        ...
        {/* Carrusel de noticias destacadas - Visible cuando filterType es 'todas' o 'noticia' */}
        {(filterType === 'todas' || filterType === 'noticia') && (
          <>
            {/* Carrusel de noticias destacadas */}
            <div 
              className="relative rounded-2xl md:rounded-3xl overflow-hidden mb-4 md:mb-6 group shadow-md shadow-[#FF8000]/10 hover:shadow-xl hover:shadow-[#FF8000]/20 transition-all duration-300"
              ...
              >
                {infiniteFeaturedNews.map((news, index) => (
                  <div 
                    ...
                    className="relative w-full flex-shrink-0 cursor-pointer group aspect-[2/1] md:aspect-[4/1]"
                  >
                    ...
                    {news.image ? (
                      ...
                      <>
                        {/* Sin imagen: fondo gris corporativo 3% con logo en 20% opacidad */}
                        <div className="w-full h-full flex items-center justify-center bg-[#45556C]/[0.03]">
                          <img 
                            src={logoDesafioSawa} 
                            alt="Logo" 
                            className="h-20 md:h-32 object-contain opacity-20"
                          />
                        </div>
                ...
                >
                  ...
                  <div className="relative w-full aspect-[21/9] overflow-hidden flex-shrink-0">
                    {article.videoUrl ? (
                      ...
                      // Imagen falló: fondo gris corporativo 3% con logo en 20% opacidad
                      <div className="w-full h-full flex items-center justify-center bg-[#45556C]/[0.03]">
                        <img 
                          src={logoDesafioSawa} 
                          alt="Logo" 
                          className="h-16 md:h-20 object-contain opacity-20"
                        />
                      </div>
                    ) : (
                      // Sin imagen: fondo gris corporativo 3% con logo en 20% opacidad
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    {/* Fecha - Ahora primero, antes del título */}
                    <div className="flex items-center gap-1.5 text-xs text-slate-500 mb-2">
                      <Calendar className="w-3.5 h-3.5" />
                      <span>{getRelativeDate(article.publishedDate)}</span>
                    </div>
                    ...
                    {/* Para Concursos: Premio en puntos */}
                    {isConcurso && article.contestPrizePoints ? (
                      <div className="flex-1 flex flex-col">
                        {/* Badge de puntos del premio */}
                        <div className="p-3 rounded-xl bg-white border-2 border-[#FF8000] flex items-center gap-2">
                          <img src={coinIcon} alt="Coin" className="w-5 h-5 flex-shrink-0" />
                          <div className="flex-1">
                            <p className="text-[10px] text-slate-500 font-medium leading-none">Se sortearán</p>
